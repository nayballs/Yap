...
Function .onInit
  ...
  ; --- PORTABLE MODE --- Auto-detect portable mode during updates so an updater
  ; run that re-launches the installer keeps the portable layout instead of
  ; silently converting it to a normal install.
  ${If} $PortableMode <> 1
  ${AndIf} $UpdateMode = 1
